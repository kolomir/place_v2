...
from _linieFormEdytuj_ui import Ui_Form
import db

class MainWindow_linieEdytuj(QWidget):
    ...
    def zapisz(self):
        ...
        if self.ui.check_aktywny.isChecked():
            ...
            aktywny = 0
        insert_data = "UPDATE linie SET nazwa = '%s', aktywne = '%s' WHERE linie.id = '%s';" % (pole_linie,aktywny,self.id_dane)
        print(insert_data)
