...
class MainWindow_raportowanie_total_prod(QWidget):
    ...
    def load_data_from_database(self):
        ...
        try:
            ...
            select_data = "SELECT * FROM `raportowanie_total` WHERE miesiac = '%s';" % (miestac_roboczy)
            ...
            for row_idx, row_data in enumerate(results):
                ...
                for col_idx, value in enumerate(row_data[1:]):  # Pomijamy id
                    ...
                    self.ui.tab_dane.setColumnWidth(9, 150)  # Stała szerokość: 150 pikseli
    ...
    def open_file_dialog(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Wybierz plik Excel", "",
                                                   "Pliki tekstowe (*.xlsx);;Wszystkie pliki (*)", options=options)
        if file_path:
    ...
    def sprawdz_wpisy(self):
        ...
        if results:
            for x in results:
                delete_data = "delete from raportowanie_total where id = '%s' and miesiac = '%s';" % (
                x[0], miestac_roboczy)
