...
# = PRODUKTYWNOŚĆ =========================================================================================================================================================
    ...
    def pokaz_dane_pracownicy(self, rows):
        ...
        for wynik in rows:
            ...
            self.ui.tab_dane_pracownicy.setItem(wiersz, 17, QTableWidgetItem(str(wynik[17])))
            wiersz += 1
        ...
        self.ui.tab_dane_pracownicy.horizontalHeader().setSectionResizeMode(17, QHeaderView.ResizeToContents)
...
# = INSTRUKTOR ======================================================================================================================================================
    ...
    def sprawdz_wpisy(self):
        ...
        select_data = "SELECT * FROM zestawienia_prod zp WHERE zp.miesiac = '{0}'".format(miestac_roboczy)  # (miestac_roboczy)
        connection = db.create_db_connection(db.host_name, db.user_name, db.password, db.database_name)
        results = db.read_query(connection, select_data)
        connection.close()
        select_data_eksport = "SELECT * FROM eksport_danych ed WHERE ed.miesiac = '{0}' and ed.dzial = 'prod'".format(miestac_roboczy)
        connection = db.create_db_connection(db.host_name, db.user_name, db.password, db.database_name)
        ...
        if results:
            connection = db.create_db_connection(db.host_name, db.user_name, db.password, db.database_name)
            for x in results:
                delete_data = "delete from zestawienia_prod where id = '%s' and miesiac = '%s';" % (x[0],miestac_roboczy)
        ...
        if result_eksport:
            ...
            for x in result_eksport:
                delete_data = "delete from eksport_danych where id = '%s' and miesiac = '%s' and dzial = 'prod';" % (x[0], miestac_roboczy)
    ...
    def zapis_dane_pracownicy(self):
        ...
        if result:
            ...
            for dane in self.lista:
                ...
                print(dane[0], dane[2], dane[3], dane[6], dane[7], dane[8], dane[9], dane[11], dane[14], data_miesiac, teraz)
                insert_data = "INSERT INTO zestawienia_prod VALUES (NULL,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');" % (dane[0], dane[2], dane[3], dane[6], dane[7], dane[10], dane[11], dane[13], dane[16], data_miesiac, teraz)
